...
"""Build and independently validate a signed distance grid for the native GRAB mug.
...
def validate(
    ...
    wp.launch(query_points, len(points), [warp_mesh.id, wp.array(points, dtype=wp.vec3, device=device)],
              outputs=[query_output], device=device)
    ...
    special_points = np.array([
        [-0.015, 0.0, 0.0],  # Center of cup cavity.
        [-0.015, 0.0, 0.040],  # Cup cavity near rim.
        [0.040, 0.0, 0.0],  # Handle opening.
        [0.050, 0.0, 0.0],  # Solid outer handle.
    ], dtype=np.float32)
    special_names = ["cup_cavity_center", "cup_cavity_near_rim", "handle_opening", "handle_material"]
    special_inside = mesh.contains(special_points)
    expected_inside = np.array([False, False, False, True])
    ...
    values = functional.grid_sample(
        torch.from_numpy(grid)[None, None], normalized[None, :, None, None],
    ...
    report = {
        ...
        "torch_gradient_norm_range": [float(x) for x in (
            np.linalg.norm(gradient, axis=1).min(), np.linalg.norm(gradient, axis=1).max()
        )],
        "special_probes": [
...
def main() -> None:
    ...
    parser.add_argument("--source", type=Path, default=local.parent / "mug_comparisons/cache/GRAB/s1__mug_lift.npz")
    ...
    if not mesh.is_watertight or not mesh.is_winding_consistent or mesh.volume <= 0:
        raise ValueError("Native mug must be an outward-oriented watertight solid")
    ...
    wp.launch(query_grid, point_count, [warp_mesh.id, wp.vec3(*lower), spacing, int(shape_xyz[0]), int(shape_xyz[1])],
              outputs=[output], device=args.device)
    ...
    np.savez_compressed(
        temporary, sdf_grid=grid, grid_min=lower, grid_max=upper, spacing_m=np.float32(spacing),
        shape_xyz=shape_xyz, canonical_vertices=vertices, canonical_faces=faces,
        ...
        sign_convention=np.asarray("negative_inside_solid_positive_empty"), **probe_arrays,
    )
    ...
    report = {
        ...
        "shape_xyz": shape_xyz.tolist(), "grid_layout": "zyx",
        "grid_min_m": lower.tolist(), "grid_max_m": upper.tolist(),
        "spacing_mm": args.spacing_mm, "padding_mm": args.padding_mm,
