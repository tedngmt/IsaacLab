...
"""Adapt the released Text2HOI refiner to all recorded GRAB mug motions.
...
def main() -> None:  # noqa: C901 - standalone experiment lifecycle with shared model state
    ...
    def predict(item: dict, start: int, end: int, raw: bool = False):
        ...
        result = model(left, right, valid_mask_lhand=mask, valid_mask_rhand=mask)
        if raw:
            return result
        return tuple(
    ...
    def local_vertices(item: dict, hand_params: tuple, start: int, end: int):
        verts = torch.cat(
            [get_hand_verts(params, layer)[0] for params, layer in zip(hand_params, layers[item["subject"]])], dim=1
        )
    ...
    report = {
        "experiment": "grab_mug_text2hoi_refiner_source_cleanup_v1",
        ...
        "loss": (
            "Native mug SDF penetration, source surface-contact preservation, "
            "vertex and temporal correction penalties"
        ),
        "unchanged": ["object poses", "frame order", "playback FPS", "personalized MANO templates"],
    ...
    for entry in entries:
        ...
        data.append(item)
        print(json.dumps({"features": clip_id, "frames": nframes}), flush=True)
    ...
    def evaluate(stage: str, export: bool = False) -> dict:
        ...
        with torch.no_grad():
            for item in data:
                ...
                if export:
                    ...
                    np.savez_compressed(
                        filename,
                        vertices_l_before=before[:, :778],
                        vertices_r_before=before[:, 778:],
                        vertices_l_after=after[:, :778],
                        vertices_r_after=after[:, 778:],
                        faces_l=layers[item["subject"]][0].faces,
                        ...
                        x_lhand_after=params[0][0].cpu().numpy(),
                    ...
                    metadata = {
                        **item["entry"],
                        ...
                        "comparison": (
                            "Original personalized MANO vs adapted Text2HOI refiner; all clips used in training"
                        ),
                        "metrics": result,
        ...
        if export:
            write_json(
                ...
                {"clips": export_entries, "source_fps": 120, "playback_fps": 30, "scope": report["scope"]},
            )
        result = {
            "per_clip": results,
            "aggregate": aggregate,
