...
def export_clip(cache: SequenceCache, dataset: str, item: dict, output_root: Path, batch_size: int) -> dict:
    ...
    arrays = {
        ...
        "object_rotation": viewer._object_rotation(dataset, sequence.motion["object_rot"][indices].reshape(-1, 3)),
    ...
    report = {
        ...
        "source_object_rotation_convention": (
            "GRAB official ObjectModel: row vertices @ Rodrigues(source_rotvec); cached active matrix is its transpose"
            if dataset == "GRAB" else "GraspXL: row vertices @ Rodrigues(source_rotvec).T"
        ),
        "units": "meters",
