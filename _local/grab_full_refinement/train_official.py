...
def main():
    ...
    protocol = dict(epochs=epochs, batch_size=args.batch_size, num_workers=0,
                    training_clips=len(loader.dataset), batches_per_epoch=batches_per_epoch,
                    effective_batch=args.effective_batch,
                    accumulation="separate penetration numerator gradients; batch-wide denominators",
                    learning_rate=config.refiner.lr, loss_weights=weights,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index, item in enumerate(loader):
            ...
            l, r, obj = (item[k] for k in ("x_lhand", "x_rhand", "x_obj"))
            ...
            with torch.no_grad():
                ...
                features = proc_obj_feat_final_train(item["cov_map"], item["obj_scale"], item["obj_cent"],
                    pointnet(item["normalized_obj_pc"]), config.texthom.use_obj_scale_centroid,
                    config.texthom.use_contact_feat)
                coarse_l, coarse_r, coarse_o = diffusion(generator, l, r, obj, features,
                    enc_text=text, get_losses=False, valid_mask_lhand=ml, valid_mask_rhand=mr,
                    valid_mask_obj=mo, ldist_map=item["ldist_map"], rdist_map=item["rdist_map"],
                    obj_verts_org=pc, obj_pc_top_idx=None)
                il, ir, ro, lc, rc, lm, rm = proc_refiner_input(coarse_l, coarse_r, coarse_o,
                    left, right, pc, normals, ml, mr, mo, item["cov_map"], "grab", return_psuedo_gt=True)
            penetration_parts.clear()
            _, _, losses = refiner.get_loss(il, ir, ro, l, r, pc, lc, rc, left, right,
                item["ldist_map"], item["rdist_map"], "grab", valid_mask_lhand=ml,
                valid_mask_rhand=mr, lhand_cont_joint_mask=lm, rhand_cont_joint_mask=rm,
                lambda_dict=weights, contact_loss_type=config.refiner.contact_loss_type)
            ...
            step += 1
            totals.append(value)
            record = dict(epoch=epoch + 1, batch=(batch_index + 1) // micro_per_step, step=step,
                          loss=totals[-1], seconds=time.monotonic() - update_start,
        ...
        state = dict(model=refiner.state_dict(), optimizer=optimizer.state_dict(), epoch=epoch,
                     loss=mean, best_loss=best_loss, step=step, protocol=protocol,
        ...
        if (epoch + 1) % config.save_pth_freq == 0:
            torch.save({"model": state["model"], "epoch": epoch, "loss": mean}, output / ("refiner_%d.pth" % (epoch + 1)))
