...
def main():
    ...
    with zipfile.ZipFile(root / "tools__object_meshes__contact_meshes.zip") as archive:
        for name in sorted({row["object_name"] for row in records}):
            ...
            objects[name] = {
                ...
                "volume_m3": float(mesh.volume), "extent_m": mesh.extents.tolist(),
                "has_pretrained_points": name in pretrained["obj_pcs"],
                "clips": sum(r["object_name"] == name for r in records),
            }
    report = {
        "clips": records, "objects": objects, "clip_count": len(records), "object_count": len(objects),
    ...
    print(json.dumps({"geometry_issues": {key: obj for key, obj in objects.items()
                                        if not (obj["watertight"] and obj["winding_consistent"]
                                                and obj["volume_m3"] > 0 and obj["has_pretrained_points"])}}))
